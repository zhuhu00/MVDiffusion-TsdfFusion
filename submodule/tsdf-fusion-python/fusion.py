...
class TSDFVolume:
  ...
  def get_mesh(self):
    ...
    # Marching cubes
    # verts, faces, norms,*rest = measure.marching_cubes(tsdf_vol, level=0, method='lewiner')[0]
    verts, faces, norms,*rest = measure.marching_cubes(tsdf_vol, level=0, method='lewiner')
    verts_ind = np.round(verts).astype(int)
